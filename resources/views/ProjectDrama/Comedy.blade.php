...
<div>
    <center> 
        <h1 class="display-4">Welcome To Genre Comedy</h1>
    </center>
</div>
@endsection
@section('content')
    <div class="container">
        ...
        @foreach($editdrama as $all)
            <div class="col-md-8">
                <a href="{{route('comedy.artikel',$all->id_kategori)}}">
                    </a>
                    @if($all->id_kategori=='4')
                    <div class="Judul">
                        <h2>{{$all->judul}}</h2>
                            <img class="img-responsive" src="{{asset('/img/comedy.jpg')}}" alt="">
                            <h3>Info Drama :
