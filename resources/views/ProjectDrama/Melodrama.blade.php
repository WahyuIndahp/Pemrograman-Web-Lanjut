...
@section('title', 'Melodrama')
<link rel="stylesheet" type="text/css" href="{{asset('/css/c.css')}}">

@section('greeting')
<div>
    <center> 
        <h1 class="display-4">Welcome To Genre Melodrama</h1>
    </center>
</div>
@endsection
@section('content')
    <div class="container">
        ...
        @foreach($editdrama as $all)
            <div class="col-md-8">
                <a href="{{route('melo.artikel',$all->id_kategori)}}">
                    </a>
                    @if($all->id_kategori=='3')
                    <div class="Judul">
                        <h2>{{$all->judul}}</h2>
                            <img class="img-responsive" src="{{asset('/img/melodrama.jpg')}}" alt="">
                            <h3>Info Drama :
